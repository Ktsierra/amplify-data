...
/**
 * Same dense bidirectional schema as p99-bidirectional-selection-set.bench.ts,
 * but with `.selectionSetDepth(1)` to validate the value proposition:
 * lower depth = fewer TypeScript instantiations.
 *
 * Compare with the depth=5 (default) benchmark to see the reduction.
 */

const selectionSet = ['name', 'spokes1.*', 'spokes2.*', 'spokes3.*'] as const;

bench(
  '10 models with bi-directional relationships - reduced depth',
  async () => {
    const schema = a
      .schema({
        Hub: a.model({
